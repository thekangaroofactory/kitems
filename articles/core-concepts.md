...
- Default arg: list of arguments to pass to the default function
  (optional)
...
- Sort rank: a number to indicate that the attribute is used to sort the
  data
...
This id is the primary key of the items, here to ensure the uniqueness
of the items.
...
function.  
You can pass another function as long as it generates unique ids (other
wise item creation will fail).
...
- access to the data model & items: (see xxx)

- initialization & integrity: (see xxx)
