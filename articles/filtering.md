...
To work on a subset of the data based on user interactions or local
filters (ex. a date range)

It’s a higher level of filtering, that is based on the intermediate
reactive object built at the pre-filtering level. If no pre-filtering is
applied, then the content of the items will be used.

The output of this level is accessible from the server function return
value `filtered_items` element.

The date slider filter provided by the package is applied at the main
filtering layer.  
If the widget is implemented in the UI, it will be automatically added
to the main filter expression(s) passed to the module server function.

This way, filters applied at the pre-filtering layer won’t need to be
computed each time a user performs filtering UI interactions (saving
computation time).
...
> goal is to display the whole list of *items* without any filtering,
> then there would be no added value compared to using the
...
module server function includes a `filters` reactive element in its
return value list.
...
helper function is delivered to build the filtering events passed to the
`filter` reactive argument.

The reason is that passing expressions to the module server function
involves some meta-programming notions that may be too complicated for
less experimented users.

If you want to know more about meta-programming with R, see [Advanced
R](https://adv-r.hadley.nz/metaprogramming.html) by Hadley Wickham.
...
The most important point to understand when it comes to the
...
[`date_slider_widget()`](https://thekangaroofactory.github.io/kitems/reference/date_slider_widget.md)
function. But it can’t guess if it’s actually going to be implemented of
not.
...
check if `filters()$date` is `NULL` or not to avoid computing the plot
twice.
