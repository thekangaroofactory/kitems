...
The names in the list should fit with the names of the corresponding
...
data model.  
This back-end process also supports creating multiple items at once. If
so, elements in the list *must* have length 1 (all items will get same
value for this attribute) or same length as the expected number of
items. If not, the process will fail as it is not safe to guess whats
the intention behind such a list.  
In case existing ids are send along the list, the corresponding items
will be skipped to avoid item duplication (it’s better to send a list
without id and let the data model generate them).

![](image/create_workflow.JPG)
...
item’s values.  
After the user confirms the update, the input values are used to update
the corresponding attributes in the item table. As usual, these values
are checked against the data model and replaced by defaults for example
if the user lets an input empty.
...
In this case, it’s possible to use the trigger to fire the update dialog
for a specific item:
...
corresponding item.  
It is possible to force the update of a computed value attribute (with
the data model default function) by including the name in the list, and
set the value to NA. In this case, the data model will be used to define
the default value.  
Multiple items update is supported as well, with same rules as for the
create workflow.  
Note that this back-end process is the only way to update multiple items
at once.

![](image/update_workflow.jpg)
...
Once the user hits the confirmation button in the dialog, the items are
deleted from the item table.
...
to a specific item (for example a marker’s popup on a map), the trigger
can be used to fire the delete confirmation dialog.
...
confirmation process as the trigger will not ask for any confirmation.
This is particularly true when the autosave is set to `TRUE` as the item
table will be saved as soon as it is modified.

![](image/delete_workflow.jpg)
