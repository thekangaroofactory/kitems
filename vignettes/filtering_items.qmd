...
The item table widget is by default a filtered view of the items - displaying the content of `filtered_items()` .

It is delivered through the `filtered_view_ui` function.

The reason for this is that if the goal is to display the whole list of items without any filtering, then there would be no added value compared to using the renderDT / DTOutput functions directly in the main Shiny application.
