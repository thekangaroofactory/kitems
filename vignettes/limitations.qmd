...
This article is dedicated to listing known limitations that are not – *at least at this time* – expected to be fixed.

- Prior to R-4.3.0, `validate()` function may raise an error when the first element of a POSIXct vector send to the trigger as `value` parameter is `NA`. This is due to missing origin in the call to as.POSIXct.
...
- Prior to kitems [v0.7.1](../news/index.html#kitems-v071-beta), there is challenge to pass multiple workflow events to the module server.
...
Multiple [events](./workflows.html#multiple-events) support solves this issue by letting the module server handle the orchestration.
