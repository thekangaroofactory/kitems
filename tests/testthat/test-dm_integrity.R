...
test_that("dm_integrity: migration @v0.5.2", {

  # -- alter data model
  dm$default.arg <- NULL
  attr(dm, "version") <- "0.5.0"

  # -- function call
  expect_message(x <- dm_integrity(data.model = dm, items = items, template = NULL))
...
test_that("dm_integrity: migration @v0.7.1", {
