...
test_that("dm_integrity: missing id (check)",
          expect_error(dm_integrity(data.model = dm[dm$name != "id", ], items = items, template = TEMPLATE_DATA_MODEL)))


test_that("dm_integrity: missing id (fix)", {

  # -- function call
  x <- dm_integrity(data.model = dm[dm$name != "id", ], items = items, template = TEMPLATE_DATA_MODEL, fix = TRUE)
